...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inptrunc = inp[:trunclen]
        # alternative: mask_len = random.binomial(n=trunclen-3, p=(trunclen-4)/(4*(trunclen-3))) + 1  
        masked_len = random.randint(int(1 / 8 * trunclen), int(3 / 8 * trunclen))
        assert trunclen >= 4, (len(inp), trunclen, masked_len, inp, idx)
        mask_start_loc = random.randint(1, trunclen-masked_len-1)
        mask_end_loc = mask_start_loc+masked_len

        prefix = inptrunc[:mask_start_loc]
        suffix = inptrunc[mask_end_loc:]
        masked_content = inptrunc[mask_start_loc:mask_end_loc]
        # construct [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] MASK_CHAR [pads]
        x_inp = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content
        x_seq = x_inp + self.PAD_CHAR * (self.block_size-len(x_inp)-1)
        assert len(x_seq) == self.block_size
        y_seq = x_seq[1:] + [self.PAD_CHAR]
        x = torch.tensor([self.stoi[con] for con in x_seq], dtype=torch.long)
        y = torch.tensor([self.stoi[con] for con in y_seq], dtype=torch.long)
        return x,y
